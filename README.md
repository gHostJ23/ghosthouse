...
| **105** | `peanut` | Network UPS Tools (NUT) | **Power Telemetry Dashboard:** Interfaces directly with the physical CyberPower UPS via USB passthrough. Hosts the PeaNUT UI to visualize active load, voltage curves, and battery runtime in real-time. |
...
| **107** | `qbittorrent` | Web GUI Port: `8080` | **Peer-to-Peer Client:** qBittorrent instance deployed for secure, localized file distribution, automation, and ISO management. |
...
* **LXC 100 (`ghostDrive`):** Deployed local SMB/Samba share on `192.168.8.129`. 
...
* **LXC 101 (`uptimekuma`):** Deployed core monitoring engine.
...
* **LXC 102 (`jellyfin`):** Deployed media streaming backend (`192.168.8.131:8096`).
...
* **LXC 103 (`adguard`):** Established primary network-wide ad-blocking engine at `192.168.8.168`.
...
* **LXC 107 (`qbittorrent`):** Deployed dedicated peer-to-peer torrent client.
